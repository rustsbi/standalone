...
type Pins_B8_B9 = (PB8<Function<6>>, PB9<Function<6>>);

impl Pins<d1_pac::UART0> for Pins_B8_B9 {}

/// D1 serial peripheral
#[derive(Debug)]
pub struct Serial<UART: Instance, PINS: Send> {
    pins: PINS,
    inner: UART,
}
...
impl<UART: Instance, PINS: Send> Drop for Serial<UART, PINS> {
    #[inline]
    fn drop(&mut self) {
        let ccu = unsafe { &*CCU::ptr() };
        UART::assert_reset(ccu);
...
impl<UART: Instance, PINS: Send> embedded_hal_nb::serial::ErrorType for Serial<UART, PINS> {
    type Error = embedded_hal::serial::ErrorKind;
}

impl<UART: Instance, PINS: Send> embedded_hal_nb::serial::Write<u8> for Serial<UART, PINS> {
    #[inline]
